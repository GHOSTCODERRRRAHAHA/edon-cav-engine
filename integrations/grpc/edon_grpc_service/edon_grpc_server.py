...
# edon_grpc_server.py is in integrations/grpc/edon_grpc_service/
...
try:
    ...
    # Fallback: define minimal message classes if protobuf not generated yet
    print("WARNING: Protobuf files not generated. Run: python -m grpc_tools.protoc ...")
...
class EdonServiceServicer(edon_pb2_grpc.EdonServiceServicer):
    ...
    def GetState(self, request, context):
        ...
        Args:
            request: StreamDataRequest with sensor data
            context: gRPC context
        
        Returns:
            StreamStateResponse with CAV state
        """
        try:
            ...
            for key, values in window.items():
                if len(values) != WINDOW_LEN:
                    ...
                    return edon_pb2.StreamStateResponse()
            ...
            # Build response
            response = edon_pb2.StreamStateResponse()
    ...
    def StreamState(self, request, context):
        ...
        Args:
            request: StreamDataRequest (stream_mode should be True)
            context: gRPC context
        
        Yields:
            StreamStateResponse updates
        """
        ...
        # For streaming, we need a buffer and periodic computation
        # This is a simplified version - in production, you'd want
        # to accumulate sensor data over time
...
def serve(port: int = 50051, max_workers: int = 10):
    ...
    # Use 0.0.0.0 for cross-platform compatibility
