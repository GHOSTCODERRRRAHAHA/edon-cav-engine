#!/usr/bin/env python3
"""
Multi-seed validation for V4 configuration.
...
def main():
    print("="*70)
    print(f"V4 Multi-Seed Validation ({N_SEEDS} seeds, {EPISODES} episodes each)")
    ...
    print(f"{'='*70}")
    print(f"\n{PROFILE.capitalize().replace('_', ' ')} V4: ~{avg_mean:.1f}% ± {avg_std:.1f}% avg improvement")
    print(f"  (Based on {N_SEEDS} seeds, {EPISODES} episodes each)")
    print(f"\nIndividual seed results:")
    for i, r in enumerate(results, 1):
        print(f"  Seed {i}: {r['average']:+.1f}%")
    
    # Save summary
    summary_file = f"results/v4_validation_{PROFILE}_summary.json"
    summary = {
        "profile": PROFILE,
        "n_seeds": N_SEEDS,
        ...
        "label": f"{PROFILE.capitalize().replace('_', ' ')} V4: ~{avg_mean:.1f}% ± {avg_std:.1f}% avg improvement"
    }
